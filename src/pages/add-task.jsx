...
import { createTask, getCategories, createCategory } from "../services/api";
...
export default function AddTask({ onTaskAdded }) {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      setErrors({
        submit: "Could not save — is json-server running on port 3001?",
      });
    } finally {
      setSubmitting(false);
    }
                ...
                {PRIORITY_OPTIONS.map((opt) => (
                  <button
                    ...
                    className={`w-full flex items-center gap-3 px-4 py-2.5 text-sm transition duration-100 ${opt.hoverBg} ${
                      priority === opt.label ? opt.activeBg : ""
                    }`}
                  >
                    <span
                    ...
                    <span className={`font-semibold ${opt.text}`}>
                      {opt.label}
                    </span>
